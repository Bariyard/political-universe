...
const AllPartiesSearch = (props: Props) => {
  ...
  const defaultFilter = {
    selectedCategories: FILTER_LIST[0].title,
    orderBy: 'NUM_EVENT',
    searchTerm: '',
  }
  const [filter, setFilter] = React.useState(defaultFilter)

  const { allPerson } = useIndividualStore((state) => state)
  const { allPersonGroup, setFilteredPersonList, filteredPersonList } = useAllPartiesStore((state) => state)

  const getCategoriesData = React.useCallback(() => {
    switch (filter.selectedCategories) {
      case FILTER_LIST[0].title: return allPerson;
      case FILTER_LIST[1].title: return allPersonGroup[FILTER_LIST[1].title];
      case FILTER_LIST[2].title: return allPersonGroup[FILTER_LIST[2].title];
      case FILTER_LIST[3].title: return allPersonGroup[FILTER_LIST[3].title];
      case FILTER_LIST[4].title: return allPersonGroup[FILTER_LIST[4].title];
      default: return allPerson;
    }
  }, [FILTER_LIST, allPersonGroup, filter.selectedCategories, allPerson])

  React.useEffect(() => {
    if (allPersonGroup) {
      setFilteredPersonList(getCategoriesData())
    }
  }, [filter.selectedCategories, FILTER_LIST, allPerson, allPersonGroup, setFilteredPersonList, getCategoriesData])

  React.useEffect(() => {
    if (filter.searchTerm !== "")
      setFilteredPersonList(getCategoriesData().filter((data) =>
        data.person.includes(filter.searchTerm)
      ))
    else {
      setFilteredPersonList(getCategoriesData())
    }

  }, [filter.searchTerm, setFilteredPersonList, getCategoriesData])


  React.useEffect(() => {
    if (filteredPersonList.length > 0) {
      let personList = filteredPersonList
      if (filter.orderBy === "ALPHABETICAL") {
        personList = personList.sort((a, b) => {
          if (a.person < b.person) { return -1; } //asc
          // if (a.person > b.person) { return 1; } //desc
          return 0;
        })
      }
      if (filter.orderBy === "NUM_EVENT") {
        personList = personList.sort((a, b) => b.count - a.count)
      }
  ...
  return (
    <div className='w-screen max-w-[480px] overflow-hidden'>
      ...
      <div className='mt-[8px]  bg-white text-neutral-super-black rounded-[10px] py-[20px] px-[12px]
          ...
          <div className=' mx-auto overflow-x-scroll'>
            <div className='flex flex-row flex-grow gap-x-[6px]'>
              <button
                className='flex-grow inline-flex items-center py-[8px] px-[12px] gap-x-[6px] bg-neutral-super-white rounded-[4px]
                      border-black border-[2px]'
                onClick={() => setFilter((prev) => ({ ...prev, orderBy: 'NUM_EVENT' }))}>
                <div className='flex-shrink-0 w-[20px] h-[20px] border-[2px] border-neutral-super-black rounded-full flex'>
                  {filter.orderBy === 'NUM_EVENT' &&
                    <div className='w-[10px] h-[10px] m-auto bg-neutral-super-black rounded-full' />
                  }
                </div>
                <div className='whitespace-nowrap text-neutral-super-black wv-ibmplex wv-u3 wv-semibold
              ...
              <button
                className='flex-grow inline-flex items-center py-[8px] px-[12px] gap-x-[6px] bg-neutral-super-white rounded-[4px]
                      border-black border-[2px]'
                onClick={() => setFilter((prev) => ({ ...prev, orderBy: 'ALPHABETICAL' }))}>
                <div className='flex-shrink-0 w-[20px] h-[20px] border-[2px] border-neutral-super-black rounded-full flex'>
                  {filter.orderBy === 'ALPHABETICAL' &&
                    <div className='w-[10px] h-[10px] m-auto bg-neutral-super-black rounded-full' />
                  }
                </div>
                <div className='whitespace-nowrap text-neutral-super-black wv-ibmplex wv-u3 wv-semibold
        ...
        <AllPartiesDataTable resetFilter={() => setFilter(defaultFilter)} />
      </div>
    </div>
  )
}
