...
const IndividualDataTable = (props: Props) => {
  ...
  return (
    <div>
      <div className='flex flex-row justify-between items-center p-[10px] mb-[6px] gap-x-1'>
        <div className='flex-shrink-0 wv-ibmplex wv-b4'>{`- ${filteredEventList.length} เหตุการณ์ -`}</div>
      ...
      {filteredEventList.length <= 0 && <div>
        <NotFound />
      </div>}
    </div>
  )
